...
export default function InfluencersPage() {
  ...
  useEffect(() => {
    const factors = getDynamicPricingFactors(mentors.length || 10)
    setDynamicFactors(factors)
    
    const interval = setInterval(() => {
      const newFactors = getDynamicPricingFactors(mentors.length || 10)
  ...
  // Map mentors to influencer-like format with dynamic pricing
  const [mappedMentors, setMappedMentors] = useState<any[]>([])

  useEffect(() => {
    const mapMentorsWithPricing = async () => {
      ...
      setMappedMentors(mapped)
    }

    if (mentors.length > 0) {
      mapMentorsWithPricing()
    } else {
      setMappedMentors([])
    }
  }, [mentors, dynamicFactors])
              ...
              <div className="flex-1 overflow-hidden relative">
                ...
                >
                  {filteredInfluencers.map((influencer, index) => (
                    ...
                    >
                      <div
                        className={`w-96 min-h-[600px] rounded-2xl border shadow-xl p-7 flex flex-col items-center justify-between transition-all duration-300 group overflow-visible mx-auto ${theme === 'dark' ? 'bg-gray-900 text-gray-100 border-gray-700' : 'bg-white text-[#222] border-gray-200'}`}
                      >
                  <div className="relative inline-block">
                  <img
                    src={influencer.avatar}
                    alt={influencer.name}
                      className="w-36 h-36 rounded-full object-cover mb-5 shadow-lg ring-4 ring-indigo-200 border-2 border-indigo-400"
                    />
                    {influencer.verified && (
                      <div className="absolute -bottom-1 -right-1 bg-blue-500 rounded-full p-1.5 border-2 border-white shadow-xl z-10">
                        <CheckCircle2 className="w-7 h-7 text-white" />
                      </div>
                    )}
                    <div className="absolute -bottom-2 left-0 flex gap-1.5">
                      {getBadges(influencer).filter(b => b.label !== 'Verified').map(b => <span key={b.label} className={`px-2.5 py-1 rounded-full text-sm font-bold ${b.color}`}>{b.label}</span>)}
                    </div>
                </div>
                  ...
                  <div className={`text-base mb-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>{influencer.handle}</div>

                  <div className={`mb-3 text-center text-sm leading-relaxed break-words ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>{influencer.description}</div>
                  ...
                  <div className="flex justify-center items-center gap-2 mb-3">
                    ...
                    <span className={`text-base font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                  ...
                  {(influencer.categories || []).slice(0, 3).map((category) => (
                    <Badge
                      key={category}
                        className={`rounded-full px-3 py-1.5 text-sm font-semibold ${theme === 'dark' ? 'bg-indigo-900 text-indigo-200' : 'bg-indigo-100 text-indigo-700'}`}
                    >
                      {category}
                    </Badge>
                  ))}
                        ...
                        {/* Pricing Display */}
                        <div className={`w-full mb-4 p-4 rounded-lg border ${theme === 'dark' ? 'bg-indigo-900/30 border-indigo-700' : 'bg-indigo-50 border-indigo-200'}`}>
                          <div className={`text-sm mb-1.5 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Starting from</div>
                          <div className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                            {influencer.displayCurrency || userCurrency.symbol}
                            {influencer.displayPrice 
                              ? influencer.displayPrice.toFixed(2)
                              : influencer.hourly_rate > 0
                              ? (influencer.hourly_rate * userCurrency.rate).toFixed(2)
                              : "0.00"}
                            <span className={`text-base font-normal ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>per hour</span>
                          </div>
                        </div>
                        ...
                        <button
                          onClick={async () => {
                            ...
                            if (user) {
                              ...
                              localStorage.setItem('tutorToBookId', influencer.id.toString());
                              window.location.href = '/dashboard/learner';
                            } else {
                              // User not authenticated, store mentor ID and open sign-in modal
                              localStorage.setItem('tutorToBookId', influencer.id.toString());
                              setIsSignInOpen(true);
                            }
                          }}
                          className={`w-full py-3 rounded-lg font-semibold text-base transition-colors ${theme === 'dark' ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-indigo-500 hover:bg-indigo-600 text-white'}`}
                        >
                          Book Tutor
                        </button>
                      </div>
