...
export function GlobeViewer({
    ...
    if (userLocation && mapInstanceRef.current) {
      try {
        ...
        if (map._loaded && map._container) {
          // Zoom to show filtered mentors (zoom level 6-8 shows a wider area)
          // If filtering by nearby, use zoom level 8 to show 30km radius better
          const zoomLevel =
            filterType === "nearby" ? 8 : filteredMentors.length > 0 ? 6 : 10;
  ...
  const mentorsWithCoords = mentors
    .map((m) => {
      const hasLat =
        m.latitude !== null &&
  ...
  console.log(
    ...
    mentorsWithCoords.length
  );
  console.log("Mentors breakdown:", {
    withExactCoords: mentorsWithCoords.filter((m) => m.hasExactCoords).length,
    withCountryCoords: mentorsWithCoords.filter((m) => !m.hasExactCoords)
      .length,
    mentorNames: mentorsWithCoords.map(
      (m) => `${m.name} (${m.country || "no country"})`
    ),
  });
  console.log(
    "All mentors data:",
  ...
  const filteredMentors = nearbyMentors.filter((mentor) => {
    // Search filter
    if (searchQuery.trim()) {
      ...
      const matchesSearch =
        ...
        mentor.specialization?.some((s) => s.toLowerCase().includes(query)) ||
        mentor.city?.toLowerCase().includes(query) ||
        mentor.country?.toLowerCase().includes(query);
    ...
    if (filterType === "online") {
      return mentor.is_online === true;
    }
    if (filterType === "offline") {
      return mentor.is_online === false;
    }
    if (filterType === "nearby") {
      // Only show mentors within 30km
      return mentor.distance !== undefined && mentor.distance <= 30;
    }

    return true; // "all" filter
  });

  // Get map center - use user location if available, otherwise center on filtered mentors or default
  const getMapCenter = (): [number, number] => {
    if (userLocation && filterType === "nearby") {
      // When filtering by nearby, center on user location
      return [userLocation.lat, userLocation.lng];
    }
    if (userLocation) {
      return [userLocation.lat, userLocation.lng];
    }
    // If we have filtered mentors with coordinates, center on them
    if (filteredMentors.length > 0) {
      const avgLat =
        filteredMentors.reduce((sum, m) => sum + m.latitude!, 0) /
        filteredMentors.length;
      const avgLng =
        filteredMentors.reduce((sum, m) => sum + m.longitude!, 0) /
        filteredMentors.length;
  ...
  const getInitialZoom = (): number => {
    ...
    if (filteredMentors.length === 1) return 6;
  ...
  useEffect(() => {
    if (filteredMentors.length > 0) {
      const initialZoom =
        userLocation && filterType === "nearby"
          ? 8
          : userLocation
          ? getInitialZoom()
      ...
      // Update map center if filter type changed
      if (mapInstanceRef.current && mapInstanceRef.current._loaded) {
        ...
        try {
          mapInstanceRef.current.setView(newCenter, initialZoom, {
            animate: true,
            duration: 0.5,
          });
        } catch (error) {
  ...
  }, [filteredMentors.length, userLocation, filterType, searchQuery]);
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {userLocation
                      ...
                      : "Searching for mentors worldwide"}
                  </p>
                </div>
                  ...
                  {isInitialized && typeof window !== "undefined" && L && (
                    <div className="w-full h-full" style={{ zIndex: 1 }}>
                      ...
                      >
                            ...
                            <Popup>
                              <div className="p-3">
                                <div className="flex items-center gap-3 mb-2">
                                  ...
                                  <div
                                    className={`w-12 h-12 rounded-full bg-blue-500 flex items-center justify-center text-white font-semibold text-lg border-2 border-blue-600 ${
                                      userAvatar ? "hidden" : ""
                                    }`}
                                  >
                                    {(userName || "You")
                        ...
                        {/* Mentor markers - Show filtered mentors */}
                      ...
                      <div className="text-center py-8">
                        <p className="text-gray-600 text-sm mb-2">
                          {searchQuery.trim() || filterType !== "all"
                            ...
                            : "No mentors with location data found"}
                        </p>
                        <p className="text-gray-500 text-xs">
                          {filterType === "nearby" && userLocation
                            ? "No mentors found within 30km of your location"
                            : mentors.length > 0
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="relative flex-shrink-0">
                                ...
                                <div
                                  ...
                                  } flex items-center justify-center text-white font-semibold text-lg border-2 border-white shadow-md ${
