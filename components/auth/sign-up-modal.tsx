...
interface SignUpModalProps {
  isOpen: boolean;
  onClose: () => void;
  isForPostingJob?: boolean; // New prop to indicate if this is for posting jobs
}

export function SignUpModal({
  isOpen,
  onClose,
  isForPostingJob = false,
}: SignUpModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Map recruiter/job_applicant and company/hiring_individual to backend types
      let userTypeForBackend = formData.userType;
      if (
        formData.userType === "recruiter" ||
        formData.userType === "company" ||
        formData.userType === "hiring_individual"
      ) {
        userTypeForBackend = "mentor"; // Recruiters go to mentors table
      } else if (formData.userType === "job_applicant") {
        userTypeForBackend = "student"; // Job applicants go to students table
      }
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          ...
          }/auth/callback?user_type=${userTypeForBackend}&full_name=${encodeURIComponent(
            formData.fullName
          )}`,
          data: {
            full_name: formData.fullName,
            user_type: userTypeForBackend,
            original_user_type: formData.userType, // Store original type for reference
          },
        },
      });
      ...
      console.log(
        "Mentor/Student record will be created after email verification in callback route."
      );
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (signUpError) {
        ...
        throw new Error(
          `Supabase authentication failed: ${signUpError.message}`
        );
      }
      ...
      // Recruiter, Mentor, Tutor, Company, or Hiring Individual should go to mentors table
      const isMentor =
        formData.userType === "recruiter" ||
      ...
      if (!existingUser) {
        ...
        if (isMentor || formData.userType === "recruiter") {
          console.log(
            "Creating mentor record for Google sign-up user:",
            data.user.id,
            "Type:",
            formData.userType
          );
          ...
          if (mentorError) {
            ...
            console.error(
              "Error details:",
              JSON.stringify(mentorError, null, 2)
            );
          ...
          formData.userType === "job_applicant"
        ) {
          // Create student record
          console.log(
            "Creating student record for Google sign-up user:",
            data.user.id
          );
      ...
      if (
        userTypeForRedirect === "recruiter" ||
        userTypeForRedirect === "mentor" ||
        userTypeForRedirect === "tutor" ||
        userTypeForRedirect === "company" ||
        userTypeForRedirect === "hiring_individual" ||
        userTypeForRedirect === "user"
      ) {
  ...
  const handleVerificationComplete = async () => {
    ...
    try {
      ...
      // Check if user is a recruiter/mentor/tutor/company/hiring_individual
      if (
        formData.userType === "recruiter" ||
        formData.userType === "mentor" ||
        formData.userType === "tutor" ||
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 flex items-center justify-center z-50">
            ...
            >
              <div className="bg-white border-2 border-blue-200 rounded-lg md:rounded-xl shadow-lg p-4 md:p-6">
                {/* Logo */}
                ...
                >
                  <div className="space-y-1.5 md:space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-1.5 md:px-4 md:py-2 bg-white border border-gray-300 rounded-md md:rounded-lg text-sm md:text-base text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  ...
                  <div className="space-y-3 md:space-y-4">
                    ...
                    {isForPostingJob ? (
                      // Show Company and Hiring Individual options for job posting
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                        <div
                          className={`relative flex items-center p-1.5 md:p-2 rounded-md md:rounded-lg border-2 cursor-pointer transition-all h-9 md:h-10 ${
                            formData.userType === "company"
                      ...
                      // Show Recruiter and Job Applicant options for regular sign-up
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                        <div
                          className={`relative flex items-center p-1.5 md:p-2 rounded-md md:rounded-lg border-2 cursor-pointer transition-all h-9 md:h-10 ${
                            formData.userType === "recruiter"
                  ...
                  <div className="flex items-start space-x-2">
                    <input
                      type="checkbox"
                      id="terms"
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-3 h-3 md:w-4 md:h-4 mt-0.5 flex-shrink-0"
                      required
                    />
                    <label
                      htmlFor="terms"
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full px-3 py-1.5 md:px-4 md:py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors font-medium disabled:opacity-50 text-sm md:text-base"
                  >
                ...
                <div className="mt-3 md:mt-4">
                  ...
                  <button
                    ...
                    className="mt-3 md:mt-4 w-full px-3 py-1.5 md:px-4 md:py-2 border-2 border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-50 text-xs md:text-sm"
                  >
