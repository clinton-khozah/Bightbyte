...
export function TutorApplicationModal({ 
  ...
  const [applicationType, setApplicationType] = React.useState<"recruiter" | "applicant">("recruiter")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!motivation.trim()) {
      newErrors.motivation = `Please provide your motivation for becoming a ${applicationType === "recruiter" ? "recruiter" : "applicant"}`
    }
    // CV is only required for applicants, optional for recruiters
    if (applicationType === "applicant" && !cvFile) {
      newErrors.cv = "CV document is required for applicants"
    }
    if (!idDocumentFile) {
    ...
    try {
      ...
      // CV is required for applicants, optional for recruiters
      if (cvFile) {
        cvUrl = await uploadFile(cvFile, "cv_document")
      } else if (applicationType === "applicant") {
        throw new Error("CV document is required for applicants")
      }
      if (idDocumentFile) {
                  ...
                  <form onSubmit={handleSubmit} className="space-y-2">
                    ...
                    <div className="space-y-0.5">
                      ...
                      <RadioGroup
                        ...
                        onValueChange={(value) => setApplicationType(value as "recruiter" | "applicant")}
                        className="flex gap-4 mt-1"
                        disabled={isSubmitting}
                      >
                        <div className="flex items-center space-x-1.5">
                          <RadioGroupItem value="recruiter" id="recruiter" className="w-3 h-3" />
                          <label htmlFor="recruiter" className="text-xs font-normal cursor-pointer">
                            Recruiter
                          </label>
                        </div>
                        <div className="flex items-center space-x-1.5">
                          <RadioGroupItem value="applicant" id="applicant" className="w-3 h-3" />
                          <label htmlFor="applicant" className="text-xs font-normal cursor-pointer">
                            Applicant
                          </label>
                        </div>
                      </RadioGroup>
                    ...
                    <div className="space-y-0.5">
                      <label className="text-xs font-medium text-gray-700 flex items-center gap-1">
                        ...
                        Why do you want to become a {applicationType === "recruiter" ? "recruiter" : "applicant"}? <span className="text-red-500">*</span>
                      </label>
                      <Textarea
                        value={motivation}
                        onChange={(e) => {
                        ...
                        placeholder={`Tell us about your motivation, experience, and what makes you a great ${applicationType === "recruiter" ? "recruiter" : "applicant"}...`}
                        rows={4}
                    ...
                    {/* CV Document / Company Document */}
                    <div className="space-y-0.5">
                      <label className="text-xs font-medium text-gray-700 flex items-center gap-1">
                        <Upload className="w-2.5 h-2.5" />
                        {applicationType === "recruiter" ? "Company Profile / Business Document" : "CV Document"}{" "}
                        {applicationType === "applicant" && <span className="text-red-500">*</span>}
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          {cvFile 
                            ? cvFile.name 
                            : applicationType === "recruiter" 
                              ? "Upload Company Profile (PDF, DOC, DOCX)" 
                      ...
                      {applicationType === "recruiter" && (
                        <p className="text-xs text-gray-500 leading-tight">
                          Upload your company profile, business registration, or other relevant business documents.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2 pt-1">
                      ...
                      <Button
                        ...
                        disabled={
                          isSubmitting || 
                          !motivation.trim() || 
                          (applicationType === "applicant" && !cvFile) || 
                          !idDocumentFile || 
                          !idNumber.trim() || 
                          !phoneNumber.trim() || 
                          !country
